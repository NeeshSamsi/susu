...
const SHAPE_ORDER = [3, 2, 1, 0] // left-to-right → path index
...
export default function LogoWave({ isPlaying }: ExperimentProps) {
  const svgRef = useRef<SVGSVGElement>(null)
  const shapesRef = useRef<SVGPathElement[]>([])
  const tlsRef = useRef<(gsap.core.Tween | gsap.core.Timeline | null)[]>([null, null, null, null])
  ...
  useEffect(() => {
    ...
    SHAPE_ORDER.forEach((shapeIdx, sliderIdx) => {
      ...
      const top = EQUAL_PRESET.tops[sliderIdx]
      const bottom = EQUAL_PRESET.bottoms[sliderIdx]
      
      gsap.set(shape, { y: 0, transformOrigin: '50% 50%' })
      
      const masterTl = gsap.timeline({ 
        delay: sliderIdx * (duration / 2) 
      })
      
      // 1. Initial movement from origin to top position
      masterTl.to(shape, { 
        y: -top, 
        duration: duration / 2, 
        ease: 'sine.out' 
      ...
      // 2. Infinite smooth loop between bottom and top
      const loopTl = gsap.timeline({ repeat: -1 })
      loopTl.to(shape, { y: bottom, duration: duration, ease: 'sine.inOut' })
      loopTl.to(shape, { y: -top, duration: duration, ease: 'sine.inOut' })
      
      masterTl.add(loopTl)
      tlsRef.current[sliderIdx] = masterTl
    })
  ...
  return (
    <>
      <svg viewBox="0 0 100 100" className="w-80 h-96" overflow="visible" fill="none" ref={svgRef}>
        {PATHS.map((d, i) => (
