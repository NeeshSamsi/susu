...
function getConfigs(p: AnimParams) {
  const half = toDuration(p.speed) / 2
  return [
    { startRot: -p.rotation, targetRot:  p.rotation, delay: 0 },
    { startRot:  p.rotation, targetRot: -p.rotation, delay: 0 },
    { startRot: -p.rotation, targetRot:  p.rotation, delay: half },
    { startRot:  p.rotation, targetRot: -p.rotation, delay: half },
  ]
}
...
export default function LogoWiggle({ isPlaying }: ExperimentProps) {
  ...
  // Mount: gather shapes, compute bboxes, start idle
  useEffect(() => {
  ...
  // Attach hover listeners to hitbox rects after bboxes are computed
  useEffect(() => {
  ...
  // Restart idle when playing state or idle params change
  useEffect(() => {
    const shapes = shapesRef.current
    ...
    <>
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 w-44 overflow-hidden flex flex-col gap-2.5 bg-[#FFFAEF] border border-[#000C31]/12 rounded-2xl px-5 py-4 shadow-sm">
        ...
        <Slider
          ...
          value={params.hoverRotation}
          min={5} max={45} step={1}
          display={`${params.hoverRotation}°`}
          onChange={v => set('hoverRotation', v)}
        />
        <Slider
          label="Hover speed"
          value={params.hoverSpeed}
          min={1} max={20} step={0.5}
          display={`${params.hoverSpeed}`}
